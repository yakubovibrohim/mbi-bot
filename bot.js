...
async function igFollowupTick() {
  ...
  for (const [uid, act] of Object.entries(igActivity)) {
    ...
    // 1) ERTA follow-up: 3-6 soat jimlik, faqat qizigan suhbatlar (4+ xabar)
    // 2) OXIRGI follow-up: 20-23.5 soat (Meta 24 soatlik oynasi yopilishidan oldin)
    let stage = null;
    if (!act.fu3 && !act.followedUp && silentH >= 3 && silentH < 6 && hist.length >= 4) { act.fu3 = true; stage = '3'; }
    else if (!act.followedUp && silentH >= 20 && silentH < 23.5) { act.followedUp = true; stage = '20'; }
    if (!stage) continue;
    try {
      const fu = await orChatMessages([
        { role: 'system', content: "Sen MBI Mebel sotuv yordamchisisan. Suhbat to'xtab qolgan mijozga BITTA qisqa, yumshoq, bosimsiz follow-up xabar yoz — mijozning tilida va yozuvida (ruscha yozgan bo'lsa ruscha, lotinda yozsa lotinda). Salom yozma (suhbat davomi), yulduzcha (**) ishlatma. Maqsad: suhbatni jonlantirish yoki telefon raqam so'rash. Faqat xabar matnini qaytar, boshqa hech narsa." },
        ...hist.slice(-10),
        { role: 'user', content: '[TIZIM: mijoz ' + stage + " soatdan beri jim. Follow-up xabar yoz.]" }
...
async function aiReply(text, userId) {
  ...
  igActivity[userId] = Object.assign(igActivity[userId] || {}, { lastClientAt: Date.now(), followedUp: false, fu3: false });
  saveIgHistoryDebounced();
  return reply;
}
