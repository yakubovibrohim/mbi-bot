...
async function aiReply(text, userId) {
  ...
  const systemCached = { role: 'system', content: [{ type: 'text', text: IG_SALES_SYSTEM, cache_control: { type: 'ephemeral' } }] };
  ...
  if (!reply) {
    ...
    reply = await groqChatMessages([{ role: 'system', content: IG_SALES_SYSTEM }, ...history], 400);
  }
  ...
  igActivity[userId] = { lastClientAt: Date.now(), followedUp: false };
  saveIgHistoryDebounced();
  return reply;
}
