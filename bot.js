...
async function aiText(system, userText, maxTokens, smart) {
  ...
  const r = await anthropicText(system, userText, maxTokens, model);
  return r || groqText(system, userText, maxTokens);
}

// Anthropic Messages API — bitta system+user matn uchun (aiText yordamchisi)
function anthropicText(systemText, userText, maxTokens, model) {
  return new Promise((resolve) => {
    ...
    const req = https.request({
      ...
      res.on('end', () => { try { const j = JSON.parse(d); resolve((j.content || []).filter(b => b.type === 'text').map(b => b.text).join('').trim() || null); } catch (e) { resolve(null); } });
    });
    req.on('error', () => resolve(null));
    req.write(body); req.end();
  });
...
async function igFollowupTick() {
  ...
  for (const [uid, act] of Object.entries(igActivity)) {
    ...
    // Eslatma FAQAT haqiqiy suhbat bo'lganda. Ilgari faqat "👏" yoki "Зўр" deb komment
    // yozgan odamlarga ham "ишлаймизми?" deb yozilar edi — bu bezovta qiladi.
    // Shart: mijozdan kamida 2 ta o'z xabari (komment seed emas) yoki 1 ta mazmunli (12+ belgi) xabar.
    const real = hist.filter(m => m.role === 'user' && !/^\[(SHAXSIY XABAR|Mijoz )/i.test(String(m.content).trim()));
    if (!real.length) continue;
    const meaningful = real.filter(m => String(m.content).replace(/[^\p{L}\p{N}]/gu, '').length >= 12);
    if (real.length < 2 && !meaningful.length) continue;
...
const igReplying = {}; // userId -> vaqt: shu mijozga hozir javob yozilyapti (ikki javob ustma-ust tushmasin)
...
## NAMUNA KO'RSATISH (tayyor ishlarimiz — reels havolasi)
[[FOTO:oshxona]] / [[FOTO:shkaf]] / [[FOTO:yotoqxona]] belgisini javob oxiriga qo'y — tizim o'zi tayyor ishlarimizdan 2 ta video havolasini yuboradi.
Qachon: mijoz rasm/namuna/tayyor ishlarni so'rasa, YOKI suhbat chuqurlashgan bo'lsa (mijoz o'lcham yoki material aytgan).
Suhbatning birinchi javobida namuna yuborma. Mijoz bizning video/postimizni ulashib yozgan bo'lsa — yuborma, u allaqachon ko'rib turibdi. Bir suhbatda ko'pi bilan 1 marta.
MUHIM: sen o'zing "rasm yubordim", "расмлар юборилди" deb YOZMA — yuborishni tizim qiladi. Sen faqat "Ҳа, кўрсатай" deb belgini qo'yasan. Sahifamizda rasm emas, VIDEO (reels) bor — shuning uchun "видео ишларимиз" деб айт.
...
Korpus LMDF, kromkalar PUR kley bilan yopishtirilgan — namlikka chidamlilik ancha yuqori. Fasadlar to'liq krashenniy, kraska Italiyaning «SIRCA» brendi, 4 etapli bo'yash — sifatga katta ta'sir qiladi. Furnitura Blum (Avstriya) — furnituraga 20 yil garantiya. Stoleshnitsa koreys sun'iy akril toshi — hech qanday styk (chok) yo'q, hammasi bir tekis, suvdan qo'rqmaydi. Dekorativ vitrina qismlari Aluframe fasadlardan. Narxi: taxminan 1 pogonaj metri 800$ atrofida. MUHIM: bu narxni aytgach DARHOL muqobil taklif qil: «byudjetga qarab arzonroq ham qilamiz — 650-700$ atrofida, yoki fasadni akrilga yo hozir trendda bo'lgan EGGER mahsulotiga o'zgartirsak sezilarli arzonlashadi». Srok: krashenniy — 30 kun.
...
// Sahifamizda faqat REELS (video) bor — rasm yo'q (22.09.2026 da tekshirildi: 28 video, 0 rasm).
// Ilgari bu funksiya faqat IMAGE qidirgani uchun HAR DOIM bo'sh qaytardi va bot mijozga
// "rasm yubordim" deb yolg'on gapirardi. Endi tayyor ishlarimizning reels havolasini beramiz.
async function igPickPhotos(category) {
  if (Date.now() - igMediaCache.ts > 24 * 3600 * 1000 || !igMediaCache.items.length) {
    const j = await httpsGetJson(`https://graph.instagram.com/me/media?fields=id,caption,media_type,permalink&limit=100&access_token=${IG_TOKEN}`);
    if (j && j.data) { igMediaCache = { ts: Date.now(), items: j.data.filter(m => m.permalink) }; }
  }
  const KW = {
    oshxona: ['oshxona', 'кухня', 'кухни', 'kitchen', 'ошхона', 'кухн'],
    shkaf: ['shkaf', 'шкаф', 'гардероб', 'prixoj', 'прихож', 'kupe', 'купе'],
    yotoqxona: ['yotoq', 'спальн', 'krovat', 'кровать', 'ётоқ']
  };
  const kws = KW[category] || [];
  const hit = igMediaCache.items.filter(m => { const c = (m.caption || '').toLowerCase(); return kws.some(k => c.includes(k)); });
  // Mos post topilmasa — eng so'nggi ishlarimizni beramiz (jim qolishdan ko'ra shu yaxshi)
  const pick = (hit.length ? hit : igMediaCache.items).slice(0, 2);
  return pick.map(m => m.permalink);
}
...
// Suhbat tarixidan qo'ng'iroq vaqtini aniqlab, iPhone kalendarga eslatma qo'yadi.
// Mijoz vaqt aytган bo'lsa — o'sha vaqt; aytmasa — bugun 12:00 (o'tган bo'lsa ertaga 12:00).
async function igScheduleCallReminder(uname, need, histText) {
  try {
    const now = tashNowStr(); // "YYYY-MM-DD HH:MM"
    const wd = new Date().toLocaleDateString('uz-UZ', { timeZone: TZ, weekday: 'long' });
    const sys = `Sen qo'ng'iroq vaqti parserisan. Hozir Toshkent vaqti: ${now} (${wd}).
Mijoz suhbatда qo'ng'iroq/aloqa uchun ANIQ vaqt aytган bo'lsa (masalan "ertaga 15:00", "тушдан кейин", "эрталаб", "kechqurun", "soat 3 da") — o'shani JSON qil: {"found":true,"date":"YYYY-MM-DD","time":"HH:MM"}.
Vaqt aniq aytilmаган bo'lsa: {"found":false}.
Qoidalar: "ertaga"=+1 kun, "indinga"=+2 kun; "эрталаб"=09:00, "тушдан кейин"/"кундузи"=14:00, "кечқурун"=18:00; hafta kuni aytilsa keyingi eng yaqin o'sha kun. FAQAT JSON qaytar.`;
    const pr = await aiText(sys, histText.slice(0, 1500), 120, false);
    const p = JSON.parse(String(pr || '{}').replace(/```json|```/g, '').trim());
    let date, time;
    if (p && p.found && p.date && p.time) {
      date = p.date; time = p.time;
    } else {
      // Standart: bugun 12:00, agar o'tган bo'lsa ertaga 12:00
      const nowT = new Date().toLocaleString('sv-SE', { timeZone: TZ });
      const todayStr = nowT.slice(0, 10);
      const hhmm = nowT.slice(11, 16);
      if (hhmm >= '12:00') {
        const d = new Date(new Date(nowT).getTime() + 24 * 3600 * 1000);
        date = d.toLocaleString('sv-SE', { timeZone: TZ }).slice(0, 10);
      } else {
        date = todayStr;
      }
      time = '12:00';
    }
    const title = `📞 ${uname}'ga qo'ng'iroq — ${need || 'mijoz'}`;
    const ok = await icloudAddReminder(title, date, time);
    return ok ? `${date} ${time}` : '';
  } catch (e) { console.error('igScheduleCallReminder:', e.message); return ''; }
}
...
async function igNotifyHotLead(from, clientText, signals) {
  ...
  const phone = pm ? pm[0] : '';
  let need = '';
  const histText = (igConvHistory[from] || []).filter(m => m.role === 'user').slice(-6).map(m => m.content).join(' | ');
  try {
    need = await aiText("Mijoz xabarlaridan nima kerakligini 3-6 so'zda ayt (masalan: oshxona 3m akril). FAQAT shu qisqa iborani qaytar.", histText.slice(0, 1500), 40, false) || '';
    need = String(need).replace(/[\r\n]+/g, ' ').trim();
  } catch (e) {}
  // Telefon bo'lsa — avtomatik qo'ng'iroq eslatmasi (iPhone kalendar)
  let remindTxt = '';
  if (phone) remindTxt = await igScheduleCallReminder(uname, need, histText);
  const txt = `🔥 *ТАЙЁР МИЖОЗ*\n👤 ${uname}\n📦 Керак: ${need || '-'}\n🎯 Сигнал: ${signals.join(', ')}${phone ? '\n📞 ' + phone : ''}${remindTxt ? '\n📅 Кўнғироқ эслатмаси: ' + remindTxt : ''}\n💬 "${(clientText||'').slice(0,150)}"\n🕐 ${new Date().toLocaleString('ru-RU',{timeZone:'Asia/Tashkent'})}`;
  try {
...
async function aiReply(text, userId) {
  ...
  // Bu uzr xabari bitta mijozga 2 soatda 1 martadan ko'p yozilmasin — 12.07.2026 da
  // ba'zi mijozlar shu jumlani ketma-ket 3-4 marta olishgan.
  if (!reply) {
    const act = igActivity[userId] || (igActivity[userId] = {});
    if (act.lastTechFail && Date.now() - act.lastTechFail < 2 * 3600 * 1000) {
      try { await msg(ADMIN, '⚠️ *IG bot yana javob bera olmadi* (mijozga uzr xabari takror yozilmadi)\nMijoz: ' + (igUsernames[userId] || userId)); } catch (_) {}
      return '';
    }
    act.lastTechFail = Date.now();
  ...
  if (reply) {
    ...
    // Modelning ichki izohlari mijozga ketmasin (14.09.2026: mijozga
    // "[Video tavsifi ko'rinmayapti, aniq loyihani aniqlab bo'lmaydi]" yuborilgan)
    reply = reply.replace(/^\s*\[[^\]\n]{10,}\]\s*$/gm, '').replace(/\n{3,}/g, '\n\n').trim();
    // "Shaxsiyga/DM ga yozing" — mijoz allaqachon DM da, bu gap mantiqsiz
    reply = reply.split('\n').filter(l => !/(шахсий|shaxsiy|лич(ку|ные)|direct|директ|\bdm\b)[^\n]{0,30}(ёзинг|yozing|напишите|пишите)/i.test(l)).join('\n').trim();
...
async function handleIGComment(c) {
  ...
  if (commenterId) {
    ...
    try {
      ...
      const oneMsg = String(reply || '').split('|||').map(p => p.trim()).filter(Boolean).join('\n\n');
      if (!oneMsg) throw new Error('javob bo\'sh — DM yuborilmadi');
...
async function igFlush(from) {
  ...
  if (!buf) return;
  // Bitta mijozga bir vaqtda ikkита javob yozilmasin (komment-DM va oddiy DM ustma-ust tushgan —
  // mijoz aralash, takroriy javoblar olgan). Band bo'lsa 6 soniyadan keyin qayta urinamiz.
  if (igReplying[from]) {
    buf.timer = setTimeout(() => { igFlush(from); }, 6000);
    console.log('IG: oldingi javob hali tugamadi, kutamiz —', from);
    return;
  }
  delete igDebounce[from];
  const combined = buf.parts.join('\n').trim();
  if (!combined) return;
  igReplying[from] = Date.now();
  try {
    await igFlushInner(from, combined);
  } finally {
    delete igReplying[from];
  }
}
async function igFlushInner(from, combined) {
  ...
  // Tayyor ishlarimiz — reels havolalari (har suhbatda 1 marta).
  // Suhbat uzunligi sharti OLIB TASHLANDI: bot "hozir ko'rsataman" deb aytib, hech narsa
  // yubormasligi mijozni ranjitgan (18.09.2026: "Расмла кани?" -> "Расмлар юборилди").
  if (fotoCat && !(igActivity[from] && igActivity[from].photoSent)) {
    try {
      const urls = await igPickPhotos(fotoCat);
      if (urls.length) {
        await igSend(from, 'Тайёр ишларимиздан:\n' + urls.join('\n'));
        igActivity[from] = Object.assign(igActivity[from] || {}, { photoSent: true });
        saveIgHistoryDebounced();
      } else {
        await igSend(from, 'Тайёр ишларимизни профилимиздаги постлардан кўришингиз мумкин 👍🏻');
      }
    } catch (e) { console.error('namuna yuborish:', e.message); }
  }
